...
def normalizar_ticks_pedidos(pedidos: List[Pedido]) -> List[Pedido]:
    """
    (Opcional, pero muy útil para presentaciones)
    Si el primer pedido se crea en tick>0, desplaza todos para que el primer pedido empiece en tick 0.
    """
    if not pedidos:
        return pedidos
    t0 = min(p.tick_creacion for p in pedidos)
    if t0 > 0:
        for p in pedidos:
            p.tick_creacion -= t0
    return pedidos
...
def guardar_heatmaps(
    grid: np.ndarray, visitas: np.ndarray, esperas: np.ndarray, prefijo: str = "heatmap"
) -> None:
    """
    Escribe:
...
def animar(
    ...
    total_frames = (ticks // pasos_por_frame) + 1

    pedidos_completados = 0
    completados_prev = set()

    def _avanzar_un_tick():
    ...
    def _redibujar():
        posiciones_local = sim.obtener_posiciones_robots()
        xs_local = [p[0] for p in posiciones_local]
        ys_local = [p[1] for p in posiciones_local]
        scat.set_offsets(np.c_[xs_local, ys_local])

        estados_local = sim.obtener_estados_robots()
        cvals_local = [estado_a_val.get(s, 0.0) for s in estados_local]
        scat.set_array(np.array(cvals_local))

        tick = sim.tick
        pedidos_totales = len(sim.pedidos)
        throughput = (pedidos_completados / (tick / 1000.0)) if tick > 0 else 0.0
    ...
    def init():
        # Frame inicial SIN avanzar (tick 0)
        return _redibujar()

    def update(frame_idx: int):
        # Tick objetivo para este frame (idempotente)
        target_tick = min(ticks, frame_idx * pasos_por_frame)
...
def main():
    ...
    agregar_argumento_config(ap)
    ap.add_argument("--seed", type=int, default=None)
    ap.add_argument("--robots", type=int, default=None)
    ap.add_argument("--ticks", type=int, default=None)
    ap.add_argument("--pasos_por_frame", type=int, default=None)
    ...
    ap.add_argument(
        "--ffmpeg_path",
        type=str,
        default=None,  # r"C:\ffmpeg\bin\ffmpeg.exe",
        help="Ruta a ffmpeg.exe (Windows). Si se omite, se usa el valor configurado en el script.",
    )

    # Normalizar tick_creacion para que el primer pedido sea tick 0 (útil para demo)
    ap.add_argument(
        "--normalizar_pedidos",
        action="store_true",
        default=None,
        help="Si se activa, desplaza tick_creacion para que el primer pedido inicie en tick 0.",
    )
    ...
    aplicar_defaults_desde_config(ap, "visualiza_simulacion")
    args = ap.parse_args()
    ...
    if args.normalizar_pedidos:
        pedidos = normalizar_ticks_pedidos(pedidos)
