...
def main():
    ...
    parser.add_argument("--pedidos", type=int, default=None)
    parser.add_argument(
        "--burst",
        action="store_true",
        default=None,
    ...
    aplicar_defaults_desde_config(parser, "generador_pedidos")
    ...
    for i in range(args.pedidos):
        ...
        if args.burst:
            if rng.random() < 0.70:
                tick_creacion = int(rng.integers(8000, 10001))  # 0 a 2000 inclusive
            else:
